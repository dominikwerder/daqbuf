...
impl CaConnSet {
    ...
    async fn run(mut this: CaConnSet) -> Result<(), Error> {
        ...
        debug!("CaConnSet beacon cancelled");
        beacons_jh.await??;
        debug!("CaConnSet beacon joined");
            ...
            if let ChannelStateValue::Active(chst2) = &mut chst.value {
                if let ActiveChannelState::WaitForStatusSeriesId { since } = chst2 {
                    ...
                    if series::dbg::dbg_chn(ch.name()) {
                        info!("addr_find_backoff set 0  {}", ch.name());
                    }
                    *chst2 = ActiveChannelState::WithStatusSeriesId(WithStatusSeriesIdState {
                    ...
                    if series::dbg::dbg_chn(name) {
                        debug!("find_ioc_query_queue.push_back  {}", name);
                    }
    ...
    fn handle_check_health(&mut self) -> Result<(), Error> {
        let selfname = "handle_check_health";
        let tsnow = Instant::now();
        let stnow = SystemTime::now();
        debug!("{selfname}");
        if self.shutdown_stopping {
    ...
    fn handle_ca_conn_eos(&mut self, addr: SocketAddr, reason: EndOfStreamReason) -> Result<(), Error> {
        let selfn = "handle_ca_conn_eos";
        debug!("{selfn}  {}  {:?}", addr, reason);
        for (chn, st1) in self.channel_states.iter() {
            match &st1.value {
                ChannelStateValue::Active(st2) => match &st2 {
                    ActiveChannelState::Init { .. } => {}
                    ActiveChannelState::WaitForStatusSeriesId { .. } => {}
                    ActiveChannelState::WithStatusSeriesId(st3) => match &st3.inner {
                        WithStatusSeriesIdStateInner::WithAddress {
                            ...
                            if SocketAddr::V4(*st_addr) == addr {
                                debug!("{selfn}  EOS  {n}", n = chn.name());
                            }
                        }
                        _ => {}
                    },
        ...
        }
        {
            ...
            match reason {
                UnspecifiedReason => {
                    warn!("{selfn}  EndOfStreamReason::UnspecifiedReason");
                    self.handle_connect_fail(addr)?
                }
                Error(e) => {
                    warn!("{selfn}  received error  {}  {}", addr, e);
                    self.handle_connect_fail(addr)?
                }
    ...
    fn ready_for_end_of_stream(&self) -> bool {
        let selfname = "ready_for_end_of_stream";
        if !self.shutdown_stopping {
            false
        } else {
            let n = self.ca_conn_ress.len();
            if n > 0 {
                debug!("{selfname}  ca_conn_ress {n}");
                false
            } else {
                let n = self.await_ca_conn_jhs.len();
                if n > 0 {
                    debug!("{selfname}  await_ca_conn_jhs {n}");
                    false
                } else {
                    true
                }
    ...
    async fn ca_conn_item_merge_inner(
        ...
        let selfn = "ca_conn_item_merge_inner";
            ...
            match item.value {
                ...
                | CaConnEventValue::Metrics(..) => {
                    if let Err(e) = tx1.send((addr, item)).await {
                        error!("{selfn}  {addr}  channel send  {:?}", e);
                        return Err(e.into());
                    }
                }
                CaConnEventValue::EndOfStream(reason) => {
                    let ts1 = Instant::now();
                    let eos_check_rx = eos_check_rx.clone();
                    tokio::spawn(async move {
                        let fut2 = async {
                            match eos_check_rx.recv().await {
                                Ok(ts2) => {
                                    let dt = ts2.duration_since(ts1);
                                    let dtms = 1e3 * dt.as_secs_f32();
                                    info!("{selfn}  {addr}  CaConn EOS dt {dtms:.0} ms");
                                }
                                Err(_) => {
                                    info!("{selfn}  {addr}  can not recv close timestamp");
                                }
                            }
                        };
                        let to = Duration::from_millis(10000);
                        match tokio::time::timeout(to, fut2).await {
                            Ok(()) => {}
                            Err(_) => {
                                info!("{selfn}  {addr}  CaConn not finished within timeout");
                            }
                        }
                    });
        ...
        let _ = eos_check_tx.send(Instant::now()).await;
        if let Some(x) = eos_reason {
            Ok(x)
        } else {
    ...
    fn check_channel_states(&mut self, tsnow: Instant, stnow: SystemTime) -> Result<(), Error> {
        let selfname = "check_channel_states";
        ...
        let it = if let Some(last) = k {
            trace!("{selfname}  check_chans  start at {:?}", last);
                    ...
                    ActiveChannelState::WithStatusSeriesId(st3) => match &mut st3.inner {
                        WithStatusSeriesIdStateInner::UnknownAddress(st4) => {
                            if st4.since + st4.backoff_dt < stnow {
                                if series::dbg::dbg_chn(&ch.name()) {
                                    debug!(
                                        "{selfname}  UnknownAddress  move to  AddrSearchPlanned  backoff {}",
                        ...
                        WithStatusSeriesIdStateInner::AddrSearchPending { since } => {
                            ...
                            if dt > SEARCH_PENDING_TIMEOUT {
                                debug!(
                                    "{selfname}  should receive some error indication instead of timeout for {:?}",
                                    ch
                                );
                            ...
                            match st4 {
                                ...
                                Assigned(st4) => {
                                    ...
                                    if st4.updated + CHANNEL_HEALTH_TIMEOUT < stnow {
                                        ...
                                        debug!("{selfname}  health timeout  channel {:?}  ~~~~~~~~~~~~~~~~~~~", ch);
                                        // TODO probably kill the whole CaConn?
                                        error!(
                                            "{selfname}  TODO  health timeout  channel {:?}  ~~~~~~~~~~~~~~~~~~~",
                                            ch
                                        );
                                        ...
                                        let snew = MaybeWrongAddressState::new(stnow, st3.addr_find_backoff);
                                        if series::dbg::dbg_chn(ch.name()) {
                                            info!("{selfname}  update state  {:?}  {:?}  {:?}", ch, addr, snew);
                                        }
                                        st3.inner = WithStatusSeriesIdStateInner::MaybeWrongAddress(snew);
                        ...
                        WithStatusSeriesIdStateInner::NoAddress { since } => {
                            if *since + NO_ADDRESS_STAY < stnow {
                                st3.inner = WithStatusSeriesIdStateInner::UnknownAddress(UnknownAddressState {
                                    since: stnow,
                                    backoff_dt: MaybeWrongAddressState::produce_backoff_dt(st3.addr_find_backoff),
                                });
                            }
                        }
                        WithStatusSeriesIdStateInner::MaybeWrongAddress(st4) => {
                            if st4.since + st4.backoff_dt < stnow {
                                if series::dbg::dbg_chn(ch.name()) {
                                    debug!(
                                        "{selfname}  MaybeWrongAddress  set to  AddrSearchPlanned  {}  backoff {}",
                                        ch.name(),
                                        st3.addr_find_backoff
                                    );
                                }
                        ...
                        match &mut st1.value {
                            ChannelStateValue::Active(st2) => match st2 {
                                ActiveChannelState::WithStatusSeriesId(st3) => {
                                    let name = ch.name();
                                    if series::dbg::dbg_chn(name) {
                                        debug!("{selfname}  issue ioc search  {:?}", ch);
                                    } else {
                                        trace!("{selfname}  issue ioc search  {:?}", ch);
                                    }
                                    search_pending_count += 1;
                                    st3.inner = WithStatusSeriesIdStateInner::AddrSearchPending { since: stnow };
                                    let (qu, rx) = IocAddrQuery::uncached_rx(name.into());
                                    self.find_ioc_query_queue.push_back((qu, rx));
                                    if series::dbg::dbg_chn(name) {
                                        debug!("{selfname}  find_ioc_query_queue.push_back  {}", name);
                                    }
                                    self.mett.ioc_search_start().inc();
                                }
                                _ => {}
        ...
        let ret = loop {
                ...
                if let Some(item) = self2.find_ioc_query_queue.pop_front() {
                    if let Some(tx) = self2.find_ioc_query_sender_2.as_mut() {
                        match tx.try_send(item.0) {
                            Ok(()) => {
                                penpro.mark_progress();
                                self2.find_ioc_rxs.push_back(item.1);
                            }
                            Err(e) => match e {
...

            {
                ...
                if let Some(rx) = self2.find_ioc_rxs.front_mut() {
                    match rx.poll_next_unpin(cx) {
                        Ready(Some(x)) => {
                            penpro.mark_progress();
                            self2.find_ioc_rxs.pop_front();
                            debug!("find_ioc_rxs  next  {x:?}");
            ...
            break if self.ready_for_end_of_stream() {
                ...
                if penpro.progress() {
                    ...
                    debug!("poll_next  ready_for_end_of_stream_with_progress");
                    continue;
                } else {
                    debug!("poll_next  EOS");
                    Ready(None)
