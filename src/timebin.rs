...
impl WithLen for Box<dyn BinningggContainerEventsDyn> {
    fn len(&self) -> usize {
        self.as_ref().len()
    }
}

impl ByteEstimate for Box<dyn BinningggContainerEventsDyn> {
    fn byte_estimate(&self) -> u32 {
        self.as_ref().byte_estimate()
    }
}

impl crate::merge::MergeableTy for Box<dyn BinningggContainerEventsDyn> {
    ...
    fn drain_into_new(&mut self, range: Range<usize>) -> crate::merge::DrainIntoNewResult<Self> {
        use crate::merge::DrainIntoNewDynResult;
        use crate::merge::DrainIntoNewResult;
        match self.as_mut().drain_into_new(range) {
            DrainIntoNewDynResult::Done(x) => DrainIntoNewResult::Done(x),
            DrainIntoNewDynResult::Partial(x) => DrainIntoNewResult::Partial(x),
            DrainIntoNewDynResult::NotCompatible => DrainIntoNewResult::NotCompatible,
        }
    }

    fn is_monotonic(&self) -> bool {
        self.as_ref().is_monotonic()
    }
